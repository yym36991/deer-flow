...
Agent `ask_clarification` → 平台向 **Owner** 要答案 → `human_input_response` → 循环（**§7.4**）。  
...
3. Owner 提交后，平台用 **同一 `X-DeerFlow-Owner-User-Id`** 调 **§7.3** 续跑
