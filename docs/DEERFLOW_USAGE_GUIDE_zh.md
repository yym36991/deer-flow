...
**人在回路（HITL）**：Agent 执行过程中暂停，等待**该 Thread 所属 Owner 用户**做出选择或输入文字；**集成平台**解析 SSE 后，用**同一 Owner** 调 `human_input_response` 续跑。

> **本部署模型（无独立业务系统）**
>
> | 角色 | 是谁 | 做什么 |
> |------|------|--------|
> | **Owner 用户** | `X-DeerFlow-Owner-User-Id`（如 `zhangsan`） | 在**你们平台**的界面里看问题、点选、输入文字 |
> | **集成平台** | 持 Internal Token 的后端 | 代 Owner 调 DeerFlow；收到 `ask_clarification` 后**向该 Owner 要答案**，再 POST 续跑 |
> | **DeerFlow** | Gateway | 暂停 / 续跑 run；**不**提供 End User 登录页 |
>
> 不存在单独的「工人账号」「工单系统账号」：HITL 里的人类就是 **Owner 本人**（或平台已代表该 Owner 拿到的选择/文本）。

DeerFlow 内置机制是 **`ask_clarification` 工具**：模型调用后 run 暂停；平台解析 SSE（或 `GET /api/threads/{id}/state`），**向当前 Owner 收集**答案后，`POST /api/runs/stream` 携带 `human_input_response` 续跑。
...
| `Content-Type` | 是 | `application/json` |
| `Accept` | 建议 | `text/event-stream` |
| `X-DeerFlow-Internal-Token` | 是 | Internal Token（平台凭证） |
| `X-DeerFlow-Owner-User-Id` | 是 | **须与触发 HITL 的 Thread 同一 Owner**（如 `zhangsan`） |
...
```text
Owner 发任务 (POST /api/runs/stream)
...
平台展示问题给 Owner → Owner 选择/输入
    → 平台 POST /api/runs/stream（同一 thread_id + Owner）
...
| 组件 | 作用 |
|------|------|
| **`ask_clarification` 工具** | 模型在信息不足、方案歧义或须 Owner 确认时**主动调用**；参数含 `question`、`clarification_type`、可选 `options` |
...
| **Owner 的续跑消息** | 新的 `POST /api/runs/stream`：`input.messages` 中一条 `role: human`，`additional_kwargs.hide_from_ui: true`，以及 `human_input_response` 绑定上一步的 `request_id` |
...
| `human_input_response.option_id` | 选择题可选；前端/UI 用，对应 SSE 里 `options[n].id`（`option-1`、`option-2`…） |

> **与 LangGraph `interrupt` 的区别**：平台日常集成**只需**本章 API。`interrupt_before` / `interrupt_after` 是图内部调试用，不等同于 `ask_clarification` 的 HITL 产品语义。

### 7.2 应用场景：对话补全 vs 动作前确认

两类需求都走**同一套 HITL API**；差异在于 Agent 为何暂停、平台如何向 **Owner** 要输入。
...
| **平台职责** | 从 SSE 取问题 → **展示给 Owner（zhangsan）** → 用其答案 POST 续跑 |
| **确定性** | **软**：模型可能该问没问 |

示例：`zhangsan` 发任务 → Agent 问「部署到哪个环境？」→ 平台 UI 让 `zhangsan` 选「测试环境」→ 平台代 `zhangsan` 续跑。

#### 7.2.2 动作前确认（须 Owner 介入后再继续）

| 维度 | 说明 |
|------|------|
| **典型问题** | 执行某操作**之前**须 Owner **确认、单选或输入**（覆盖文件、调外部 API、批量重试等） |
| **业务要求** | 卡点在 SOUL/Skill 或平台拆 run 中约定，不能仅靠模型「自觉」 |
...
```text
平台代 zhangsan 发：「对订单 #123 执行重试」
    → Agent ask_clarification（risk_confirmation，options: [执行, 取消]）
    → run 暂停
    → 平台 App：zhangsan 点「执行」
    → 平台 POST 续跑（Owner 仍为 zhangsan，human_input_response）
    → Agent 继续后续工具 / MCP
```

| clarification_type | Owner 交互 | 示例 |
|--------------------|------------|------|
...
| 能力 | 说明 |
|------|------|
| **支持** | 暂停 run；Owner **选择/输入**；同 Thread 多轮 HITL；`GET .../state` 轮询 pending |
| **不支持（开箱）** | 按业务「节点名」自动停；用 **§7.2.4** 实现 |
| **`interrupt_before` / `interrupt_after`** | LangGraph 图内部调试用；平台日常**不必用** |
| **Guardrails** | 工具 **ALLOW/DENY**，**不暂停**等人输入 |
| **定时 / 非交互 run** | 无 `ask_clarification`；须拆 run，不能靠本章 HITL |

#### 7.2.4 推荐集成模式（仅平台 + Owner）

**模式 A — Agent + SOUL（软保证）**

SOUL 写明：涉及 {动作} 必须先 `ask_clarification`（`risk_confirmation` + `options`）。  
平台仍负责把问题交给 **Owner** 并续跑。

**模式 B — 平台拆段 run（硬卡点，推荐）**

```text
Run1: Agent 输出「待 zhangsan 确认」的结构化结果 → run 结束
平台: 在你们的 App 里展示给 zhangsan，收集选择/文字（可存你们库）
Run2: 同一 thread_id + Owner=zhangsan，POST 续跑（messages 带 zhangsan 的结论）
```

卡点由**平台编排**保证；DeerFlow 只做两段推理。

**模式 C — 同 Thread 多轮 HITL（API 原生）**

Agent `ask_clarification` → 平台向 **Owner** 要答案 → `human_input_response` → 循环（**§7.3**、**§7.6**）。  
...
1. 监听 SSE / 轮询 state → `human_input_request`
2. **关联到当前 Owner 的会话**（你们 App/IM/表单），展示 `question` / `options`
3. Owner 提交后，平台用 **同一 `X-DeerFlow-Owner-User-Id`** 调 **§7.5** 续跑
4. **`thread_id` 不变**；换 Owner 会 404 / 续跑失败

> 平台也可以**代填**答案（如 Owner 已在你们系统里点过按钮，平台直接 POST），但 **`Owner` 头必须与 Thread 创建时一致**。
...
**场景**：Owner 要求 Agent 部署服务；Agent 不确定环境时必须先问，不能猜测。实际跑通时 Agent 常会**连续多轮**追问（环境 → 服务器类型 → 部署方式等），每轮流程相同。
...
Agent 收到答案后会继续推理。**若仍缺信息，会再次 `ask_clarification`**（多轮 HITL）。例如昨天实测（`thread_id: hitl-api-test-002`）在 Owner 选「测试环境」后，Agent 又依次问了：

```text
Round 1  目标环境？       → Owner：测试环境   (request_id: clarification:call_0NRv_...)
Round 2  服务器类型？     → Owner：Windows    (request_id: clarification:call_HdOA_...)
Round 3  部署方式？       → Owner：手动部署
```

每一轮 SSE 末尾都会出现**新的** `request_id`；平台须从**最新** pending 问题取值，不能用历史 ID。
...
每一轮重复：**解析最新 SSE → 向 Owner 展示 → POST 续跑**。`hitl-step3-reply.json` 示例为 Owner 选 **Windows**：
...
```text
Round 1  目标环境？     → Owner：测试环境
Round 2  服务器类型？   → Owner：Windows
Round 3  部署方式？     → Owner：手动部署 / 自动化部署
         → Agent 继续给出后续步骤或调用工具
```

**判定是否仍有 pending**：遍历 `messages`，找出所有 `human_input_response.request_id` 已回复的 ID；最后一条 `ask_clarification` 的 `request_id` **不在**已回复集合中 → 仍需 Owner 输入。
...
while True:
    sse = post_runs_stream(messages=[...], thread_id=thread_id, owner="zhangsan")
    pending = parse_last_ask_clarification(sse)  # artifact.human_input
    if not pending:
        break  # run 正常结束，无待回复问题
    answer = show_ui_to_owner(pending["question"], pending["options"])
    post_runs_stream(
        messages=[{
            "role": "human",
...
**示例 A — 对话补全**（Owner=`zhangsan`，平台代调 API）：

```
Step1  平台代 zhangsan POST runs/stream → Agent ask_clarification（目标环境）
Step2  平台解析 SSE，在 App 里让 zhangsan 选择 → 代 zhangsan POST 续跑 → Agent 再问
Step3  平台再次代 zhangsan 回复 → Agent 继续
```

**示例 B — 动作前确认**（Owner=`zhangsan`）：

```
Step1  平台代 zhangsan：「对批次 B-2026 重新质检」→ Agent ask_clarification
        （risk_confirmation，options: [确认执行, 取消]）
Step2  平台 App：zhangsan 点「确认执行」→ 平台 POST human_input_response（Owner=zhangsan）
...
| 3 | `option_id` 按 SSE 中 options 顺序（`option-1`、`option-2`…）；后端不校验 option_id 与 value 是否匹配，集成层自行保证。**解析 `tool_call_id` 时须取 `artifact.human_input.tool_call_id` 或 AI `tool_calls[].id`，不要误取 options 里的 `option-1`** |
